...
class LongThrower(ThrowerAnt):
    ...
    min_range = 4
    def nearest_bee(self, hive):
        next_place = self.place.entrance.entrance.entrance.entrance.entrance
        while next_place != hive:
            if next_place.bees != []:
                return random_or_none(next_place.bees)
            next_place=next_place.entrance
        return random_or_none(self.place.bees)
#        super(LongThrower,self).nearest_bee(hive)

#        if next_place == hive or next_place == None:
#            return None
...
class ShortThrower(ThrowerAnt):
    ...
    max_range = 3
    def nearest_bee(self,hive):
#        super(ShortThrower,self).nearest_bee(hive)
        i=1
        next_place = self.place.entrance
        while i<= self.max_range and next_place != hive :
            i+=1
            if next_place.bees != []:
                return random_or_none(next_place.bees)
            next_place=next_place.entrance
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting != 0:
            ...
            return self.eat_bee(random_or_none(colony.bees))
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        after_queen = self.place.exit
#        a = colony.places
#        for i in a:
#            if isinstance(i.place.ant,QueenAnt)
#                return True

        if self.impostor > 1:
            self.reduce_armor(self.armor)
        else:
            while after_queen != None:
                if after_queen.ant != None:
                    after_queen.ant.damage = after_queen.ant.damage*2
                    self.bee_inspired += [after_queen.ant]
                after_queen =  after_queen.exit
